...
class JdbcDriverAdd extends AbstractAddStepHandler {
   static final JdbcDriverAdd INSTANCE = new JdbcDriverAdd();

   private JdbcDriverAdd() {
      super(Arrays.asList(JdbcDriverResource.JDBC_DRIVER_ATTRIBUTES));
   }
...
//
//      model.get(NAME.getName()).set(driverName);
//   }
   ...
   static void startDriverServices(final ServiceTarget target, final ModuleIdentifier moduleId, Driver driver, final String driverName,
      ...
      final ServiceBuilder<Driver> builder = target.addService(ServiceName.JBOSS.append(ModelKeys.JDBC_DRIVER, driverName.replaceAll("\\.", "_")), driverService)
            .addDependency(DriverRegistryService.SERVICE_NAME, DriverRegistry.class,
