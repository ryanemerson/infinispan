...
public interface DatasourcesLogger extends BasicLogger {
   ...
   @LogMessage(level = INFO)
   @Message(id = 21, value = "Started DataSource service with pool-name = %s")
   void startedDataSourceService(String poolName);

   @LogMessage(level = INFO)
   @Message(id = 22, value = "Stopped DataSource service with pool-name = %s")
   void stoppedDataSourceService(String poolName);
}
