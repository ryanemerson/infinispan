package org.infinispan.persistence.jdbc.stores.binary;
 ...
 * in database, each row corresponding to a bucket. This is in contrast to {@link org.infinispan.persistence.jdbc.stores.stringbased.JdbcStringBasedStore}
 * which stores each StoredEntry as a row in the database. </p> It is generally recommended to use {@link
 * org.infinispan.persistence.jdbc.stores.stringbased.JdbcStringBasedStore} whenever possible as it performs better. Please
 * read {@link org.infinispan.persistence.jdbc.stores.stringbased.JdbcStringBasedStore}'s javadoc for more details on this.
 * <p/>
 ...
 * <p/>
 * All the DB related configurations are described in {@link org.infinispan.persistence.jdbc.stores.binary
...
public class JdbcBinaryStore<K,V> extends AbstractJdbcStore<K,V> {
   ...
   private Set<Bucket> updateAndCreateBuckets(Collection<MarshalledEntry> modifiedEntries, Set<Object> keysToDelete,
      ...
      for (MarshalledEntry entry : modifiedEntries) {
         ...
         // How should we handle expired entries in a Tx?
         if (m != null && m.isExpired(ctx.getTimeService().wallClockTime())) {
