...
import static org.infinispan.persistence.jdbc.configuration.TableManagerConfiguration.*;

/**
 * TableManagerConfigurationBuilder.
 *
 * @author Tristan Tarrant
 * @since 5.2
 */
public abstract class TableManagerConfigurationBuilder<B extends AbstractJdbcStoreConfigurationBuilder<?, B>, S extends TableManagerConfigurationBuilder<B, S>> extends
                                                                                                                                                                          AbstractJdbcStoreConfigurationChildBuilder<B> implements Builder<TableManagerConfiguration>, Self<S> {
   private static final Log log = LogFactory.getLog(TableManagerConfigurationBuilder.class, Log.class);
   private final AttributeSet attributes;

   TableManagerConfigurationBuilder(AbstractJdbcStoreConfigurationBuilder<?, B> builder) {
      super(builder);
      attributes = TableManagerConfiguration.attributeSet();
   }
   ...
   @Override
   public TableManagerConfiguration create() {
      return new TableManagerConfiguration(attributes.protect());
   }

   @Override
   public Builder<?> read(TableManagerConfiguration template) {
   ...
   public String toString() {
      return "TableManagerConfigurationBuilder [attributes=" + attributes + "]";
   }
