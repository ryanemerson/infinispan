...
public class StateReceiverTest extends AbstractInfinispanTest {
   ...
   public void testGetReplicaException() throws Exception {
      ...
      when(task.requestSegments()).thenReturn(createExceptionalCompletableFuture());
      doReturn(task).when(stateReceiver).createTransferTask(any(Integer.class), any(Address.class));
      CompletableFuture<List<Map<Address, InternalCacheValue<Object>>>> cf = stateReceiver.getAllReplicasForSegment(0);
      try {
         cf.get();
   ...
   public void testTopologyChangeDuringSegmentRequest() throws Exception {
      ...
      // Reduce #nodes to less than numowners to force a rebalance
   ...
   public void testOldAndInvalidStateIgnored() {
      ...
      Map<Object, Map<Address, InternalCacheValue<Object>>> receiverKeyMap = stateReceiver.getKeyReplicaMap();
   ...
   private CompletableFuture<Void> createExceptionalCompletableFuture() {
      CompletableFuture<Void> cf = new CompletableFuture<>();
      cf.completeExceptionally(new CacheException("Problem encountered retrieving state"));
      return cf;
   }
