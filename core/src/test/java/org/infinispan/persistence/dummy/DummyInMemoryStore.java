...
public class DummyInMemoryStore implements AdvancedLoadWriteStore, AdvancedCacheExpirationWriter {
   ...
   public void purge(Executor executor, ExpirationPurgeListener listener) {
      ...
      for (Iterator<Map.Entry<Object, byte[]>> i = store.entrySet().iterator(); i.hasNext();) {
         ...
         MarshallableEntry se = deserialize(next.getKey(), next.getValue(), true);
   ...
   public MarshallableEntry loadEntry(Object key) {
      ...
      MarshallableEntry me = deserialize(key, store.get(key), true);
   ...
   public Flowable<MarshallableEntry> entryPublisher(Predicate filter, boolean fetchValue, boolean fetchMetadata) {
      ...
      return flowable.compose(f -> {
         ...
         return f.map(entry -> deserialize(entry.getKey(), entry.getValue(), fetchValue))
   ...
   public void blockUntilCacheStoreContains(Object key, Object expectedValue, long timeout) {
      ...
      while (timeService.wallClockTime() < killTime) {
         MarshallableEntry entry = deserialize(key, store.get(key), true);
   ...
   public boolean contains(Object key) {
      ...
      MarshallableEntry me = deserialize(key, store.get(key), false);
   ...
   private byte[] serialize(MarshallableEntry o) {
      try {
         return marshaller.objectToByteBuffer(new Entry(o.getValue(), o.metadata(), o.created(), o.lastUsed()));
      } catch (IOException e) {
   ...
   private MarshallableEntry deserialize(Object key, byte[] b, boolean fetchValue) {
      try {
         if (b == null)
            return null;
         // We have to fetch metadata to tell if a key or entry is expired. Note this can be changed
         // after API changes
         Entry entry = (Entry) marshaller.objectFromByteBuffer(b);
         return ctx.getMarshallableEntryFactory()
               .create(key, (fetchValue ? entry.value : null), entry.metadata, entry.created, entry.lastUsed);
