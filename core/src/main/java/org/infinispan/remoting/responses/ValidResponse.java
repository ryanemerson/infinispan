...
public abstract class ValidResponse implements Response {
   ...
   @ProtoField(number = 1)
   protected final MarshallableObject<?> object;

   @ProtoField(number = 2)
   protected final MarshallableCollection<?> collection;

   // TODO do we need MarshallableMap for this to handle internal types?
   @ProtoField(number = 3)
   protected final MarshallableMap<?, ?> map;

   @ProtoField(number = 4)
   protected final MarshallableArray<?> array;
