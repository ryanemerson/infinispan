...
public class AsyncCacheWriter extends DelegatingCacheWriter {
   ...
   public void start() {
      ...
      // If no errors on start, then start observing the status of the underlying store. We monitor the availability of
      // the delegate cache store here, opposed to the PersistenceManager, as it's possible for AsyncCacheWriter to be
      // availabile even if the delegate isn't, this occurs when the modification queue is still able to queue requests
      long interval = ctx.getCache().getCacheConfiguration().persistence().availabilityInterval();
      Observable.interval(interval, TimeUnit.MILLISECONDS, Schedulers.from(ctx.getExecutor()))
            .map(t -> actual.isAvailable())
            .doOnError(err -> log.debugf("Error encountered when calling isAvailable on %s: %s", actual, err))
   ...
   public boolean isAvailable() {
      // Available if failSilently || actual == available || actual != available and queue has capacity
      // Worst case, writeBatch comes in before !Available is declared by Async, in which case it will wait on
      // writeLock until the stateLock is reset when the queue is finally flushed
      return !stopped && (asyncConfiguration.failSilently() || delegateAvailable || stateLock.hasCapacity());
   }
