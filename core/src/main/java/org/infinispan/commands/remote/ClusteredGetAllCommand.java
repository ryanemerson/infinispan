...
public class ClusteredGetAllCommand<K, V> extends BaseClusteredReadCommand {
   ...
   public CompletableFuture<Object> invokeAsync(ComponentRegistry registry) throws Throwable {
      if (!hasAnyFlag(FlagBitSets.FORCE_WRITE_LOCK)) {
         return invokeGetAll(registry);
      } else {
         return acquireLocks(registry.getCommandsFactory())
               .thenCompose(o -> invokeGetAll(registry));
      }
   }

   private CompletableFuture<Object> invokeGetAll(ComponentRegistry cr) {
   ...
   private CompletableFuture<Object> acquireLocks(CommandsFactory commandsFactory) throws Throwable {
