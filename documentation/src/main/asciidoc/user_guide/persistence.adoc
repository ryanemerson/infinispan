...
please consult the link:http://docs.jboss.org/infinispan/{infinispanversion}/apidocs/[javadoc].
...
be provided by a properties file, either via placing a `hikari.properties` file on the classpath or by specifying it's
location by the system property `-Dhikaricp.configurationFile=`. N.B. When both `hikaricp.configurationFile` and
`hikari.properties` are provided, only the properties of the former are loaded. Refer to the official link:https://github.com/brettwooldridge/HikariCP[documentation]
for details of all configuration properties.
