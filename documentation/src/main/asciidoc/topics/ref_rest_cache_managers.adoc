...
If no additional request parameters are provided, then all resources in the Cache Manager are included in the generated backup.
However, if one or more resource types are specified as parameters then only the specified resources are included. For
example, the following parameters `?action=backup&caches=cache1,cache2&cache-configs=*` would result in the backup file
containing all defined cache configurations and the caches `cache1` and `cache2`.
...
|`caches`
|OPTIONAL
|A csv list of cache names to backup, or "*" for all

|`cache-configs`
|OPTIONAL
|A csv list of cache configuration to backup, or "*" for all

|`counters`
|OPTIONAL
|A csv list of counter names to backup, or "*" for all

|`proto-schemas`
|OPTIONAL
|A csv list of protobuf schema names to backup, or "*" for all

|`scripts`
|OPTIONAL
|A csv list of scripts names to backup, or "*" for all

|===

[id='rest_v2_restore_manager']
= Restore {brandname} Cache Manager content
Restore the content matching this managers name from a backup file to this Cache Manager.

[source,options="nowrap",subs=attributes+]
----
POST /rest/v2/cache-managers/{cacheManagerName}?action=restore
----

If no additional request parameters are provided, then all of the resources in the uploaded backup file are restored to
this Cache Manager instance. However, if one or more resource types are specified as parameters then only the specified
resources are restored. For example, the following parameters `?action=restore&caches=cache1` would result in only the
cache with name "cache1" being restored to this Cache Manager.
...
and then restored to the Cache Manager.
