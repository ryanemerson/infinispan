...
= Create a Backup of {brandname} Cache Manager content
...
that are currently stored in the cache manager. The created backup file can then be retrieved via a subsequent
...
If no backup parameters are provided in the body of the request, then all resources in the Cache Manager are included
in the generated backup. However, if one or more resource types are specified via a json object in the request body,
then only the specified resources are included.
...
The following example shows how to specify that all counters and only caches with the names `[cache1,cache2]` should
be included in the created backup.
...
application/zip::
To be used if the backup file should be restored in it's entirety.
...
multipart/form-data::
Allows the backup zip file to be uploaded alongside a JSON parameters object, allowing only a subset of the
backup files content to be restored.
...
|OPTIONAL
|A JSON object of request parameters.
...
The following example shows how to specify that only counter resources should be restored from the uploaded backup.
