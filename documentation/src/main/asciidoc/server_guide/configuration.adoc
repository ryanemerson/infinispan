...
pom. Custom schemas for object marshalling must then be registered with the selected library using the libraries api on
...
marshalling begins. This can be achieved via the helper method `KryoMarshaller::registerSerializer(..)` which registers
the custom schema with the underlying Kryo object used for marshalling.  If more complex registration, or manipulation of
the underlying Kryo instances is required, you can access the Kryo instance via `KryoMarshaller::getKryo()`.

For example, when registering a schema on the HotRod client the following steps are required:
...
ConfigurationBuilder builder = new ConfigurationBuilder();
      ...
      KryoMarshaller marshaller = remoteCacheManager.getMarshaller();
      marshaller.registerSerializer(ExampleObject.class, new ExampleObjectSerializer());
----

Alternatively, you can implement a service provider for the `SerializerRegistryService.java` interface, placing all serializer registrations
in the `register(Kryo)` method, where the serializers should be applied to the provided `Kryo` object via the Kryo api.
...
http://central.maven.org/maven2/org/infinispan/infinispan-marshaller-kryo-bundle/{infinispanversion}/infinispan-marshaller-kryo-bundle-{infinispanversion}.jar
...
be placed on the classpath of the server.  To achieve this users can follow the steps outlined in the protocol interoperability
