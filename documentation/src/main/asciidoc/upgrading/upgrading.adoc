...
Additional properties for HikariCP can be provided by placing a `hikari.properties` file on the classpath or by using the
system property `hikaricp.configurationFile` to specify the location of a hikari properties file. N.B. When both
`hikaricp.configurationFile` and `hikari.properties` are provided, only the properties of the former are loaded.

Support for c3p0 has been deprecated and will be removed in a future release.

Users can force c3p0 to always be utilised by providing the system property `-Dc3p0.force=true`. However, if no hikari
property file exists (on the classpath or via the system property) and there is a `c3p0.properties` or `c3p0-config.xml` file
on the classpath, then c3p0 will be utilised without needing to specify `c3p0.force=true`.
