...
import org.infinispan.commons.util.logging.processor.apt.report.ISPNXmlReportWriter;
...
@SupportedOptions({
   ISPNReportFileGenerator.REPORT_TYPE,
   ISPNReportFileGenerator.REPORT_PATH,
   ISPNReportFileGenerator.REPORT_TITLE
})
public class ISPNReportFileGenerator extends AbstractGenerator{
   ...
   ISPNReportFileGenerator(final ProcessingEnvironment processingEnv) {
       super(processingEnv);
       ...
       if (reportType != null) {
           try {
               ...
               try (
                       ...
                       final ISPNXmlReportWriter reportWriter = new ISPNXmlReportWriter(messageInterface,writer);//= ReportWriter.of(reportType, messageInterface, writer)
               ) {
